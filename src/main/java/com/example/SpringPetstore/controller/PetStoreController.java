...
public class PetStoreController {
    ...
    @ResponseBody
    public Pet addPet(@RequestParam String name) {
        return petService.addPet(Pet.builder().name(name).status("AVAILABLE").build());
    }
    ...
    @ResponseBody
    public Order addOrder(@RequestParam Integer quantity, @RequestParam LocalDate ship_date) {
        return orderService.addOrder(Order.builder().quantity(quantity).shipDate(ship_date).status("DUMMY").complete(false).build());
    }
